...
function areUnlockItemsEqual(previousItem, nextItem) {
  ...
  return (
    ...
    previousItem.unlockProgress?.knowledge ===
      nextItem.unlockProgress?.knowledge
  )
}
...
export const UnlockCard = memo(
  function UnlockCard({ title, item, accentClass }) {
    ...
    const shishkiPct = Math.min(
      100,
      (item.unlockProgress.shishki / Math.max(1, item.unlockRule.shishki)) *
        100,
    )
    const knowledgePct = Math.min(
      100,
      (item.unlockProgress.knowledge / Math.max(1, item.unlockRule.knowledge)) *
        100,
    )
